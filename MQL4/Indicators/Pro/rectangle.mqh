...
//|                                                                  |
//+------------------------------------------------------------------+
bool RectangleChange(const long            chart_ID=0,        // chart's ID
                     const string          name="Rectangle",  // rectangle name
                     const int             sub_window=0,      // subwindow index
                     const color           clr=clrRed,        // rectangle color
                     const ENUM_LINE_STYLE style=STYLE_SOLID, // style of rectangle lines
                     ...
                     const long            z_order=0)         // priority for mouse click
  {
   double pricerange = ChartGetDouble(0, CHART_PRICE_MAX, 0) - ChartGetDouble(0, CHART_FIXED_MIN, 0);
   double maxPrice = ChartGetDouble(0, CHART_PRICE_MAX, 0) + pricerange;
   double minPrice = ChartGetDouble(0, CHART_FIXED_MIN, 0) - pricerange;
   int timestart = (int)ObjectGetInteger(0, name,OBJPROP_TIME, 0);
   int timeend = (int)ObjectGetInteger(0, name,OBJPROP_TIME, 1);
   
   RectanglePointChange(0,name, 0,(datetime)timestart,maxPrice);
   RectanglePointChange(0,name, 1,(datetime)timeend,minPrice);
//--- set anchor points' coordinates if they are not set
//--- set rectangle color
   ObjectSetInteger(chart_ID,name,OBJPROP_COLOR,clr);
//--- set the style of rectangle lines
