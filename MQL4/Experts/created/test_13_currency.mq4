...
///////////////////////real/////////////////////////////////////
   ...
   tradecondition[rsi1] = RSI_check2(rsi1, 5, 25, 75, tradecondition[rsi1]);

   tradecondition[rsi2] = RSI_check1(rsi2, 15, 30, 70, 2); //return if in buy or sell region

   tradecondition[rsi3] = RSI_check1(rsi3, 30, 40, 60, 1);  //return if in buy or sell region

   tradecondition[stoch1] = STOCH_check1(stoch1, 7, 5, 3, 20, 80, tradecondition[stoch1]);     //new
//975 = 975 - 0
   tradecondition[stoch2] = STOCH_check2(stoch2, 7, 5, 3, 20, 80, tradecondition[stoch2]);     //old

   tradecondition[moch1] = MOCH_check1(moch1, 5, 10, 5, tradecondition[moch1]);

   tradecondition[moch2] = MOCH_check2(moch2, 5, 10, 5);

   tradecondition[avg1] = AVG_check1(75, 0, 0.0001);
...
//      (tradecondition[rsi2] == buy) &&
      (tradecondition[rsi3] == buy) &&
//(tradecondition[moch2] == buy) &&
//(tradecondition[avg1] == trade) &&
      (tradecondition[time_gap] == 1) &&
      (
         (tradecondition[rsi1] == buytrigger)
         || (tradecondition[stoch1] == buytrigger)
//    || (tradecondition[stoch2] == buytrigger)
//         (tradecondition[moch1] == buytrigger)
      ))
     {
      double size = tradeallow*(InpOrderSize + 0.005 * buycount + (conditionvalues[rsi3])/1000);
...
//    (tradecondition[rsi2] == sell) &&
      (tradecondition[rsi3] == sell) &&
//(tradecondition[moch2] == sell) &&
//(tradecondition[avg1] == trade) &&
      (tradecondition[time_gap] == 1) &&
      (
         (tradecondition[rsi1] == selltrigger)
         || (tradecondition[stoch1] == selltrigger)
//      (tradecondition[stoch2] == selltrigger)
//       (tradecondition[moch1] == selltrigger)
      ))
     {
      double size = tradeallow*(InpOrderSize + 0.005 * sellcount + (conditionvalues[rsi3])/1000);
...
//|                                                                  |
//+------------------------------------------------------------------+
int AVG_check1(int period, int shift, double gradient)
  {
   double avg1[10];
   double avg[10];
   for(int i=0; i<10; i++)
     {
      avg1[i] = iMA(Symbol(),Period(),period,shift,MODE_EMA,PRICE_CLOSE,i+1);
      avg[i] = avg1[i]/avg1[0] -1;
     }
//printf("%f %f %f %f %f %f %f %f %f %f", avg[0],avg[1], avg[2],avg[3], avg[4],avg[5], avg[6],avg[7],avg[8],avg[9]);
