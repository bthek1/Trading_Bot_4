...
//    || (tradecondition[stoch2] == buytrigger)
     ...
     {
      double size = InpOrderSize + 0.005 * buycount + (conditionvalues[rsi3])/1000;
...
//      || (tradecondition[stoch2] == selltrigger)
     ...
     {
      double size = InpOrderSize + 0.005 * sellcount + (conditionvalues[rsi3])/1000;
...
int AVG_check1(int period, int shift)
  {
   ...
   if(fabs(AVG1 - AVG2) < (AVG2*0.001))
     {
      Alert(fabs(AVG1 - AVG2), "  trade  ",(AVG2*0.000001));
      return trade;
     }
   Alert("no");
   return notrade;
